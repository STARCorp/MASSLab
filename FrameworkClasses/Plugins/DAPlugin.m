classdef (Abstract) DAPlugin < MASSPlugin
...
%     Ph.D. Dissertation, University of Massachusetts Dartmouth, Jan. 2019.
    ...
    properties (Hidden)
        % Data from SEM
        semObsExtSig; %ExternalSignal []
        semSrcExtSig; %ExternalSignal []
        semBlkLen; % Block length in samples
        semBlkStep; % Block step in samples
        semSampRate; % Sampling Rate
        srcObs01Dly; % Vector of sample delays from each source's 1st 
        % appearance in any observation, so that this delay represents
        % zero lag in the first appearance.
    end
    ...
    methods 
        % Constructor
        function obj=DAPlugin()
            obj.superClassHidden = 'DAPlugin';
            obj.pluginStatus = 0;
        end
